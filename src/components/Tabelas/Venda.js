...
function TabelaVenda() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`localhost:8000/venda/${postoId}/vendasPorPosto/`);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`localhost:8000/venda/${id}`);
            const resposta = await axios.get('localhost:8000/venda/');
